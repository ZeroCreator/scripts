...
if [ ! -f "$TIMESTAMP_HOST_FILE" ]; then
    send_telegram_warning_message "❌ FATAL!!! 🤬 $(date '+%Y-%m-%d %H:%M:%S') - Файл $TIMESTAMP_HOST_FILE не найден."
    exit 1
fi
...
if (( (current_time - last_modified_time) > (TIME_DELTA * 3600) )); then
    ...
    send_telegram_warning_message "❌ FATAL!!! 🤬 $(date '+%Y-%m-%d %H:%M:%S') - Не было сообщений о работе грабберов за $TIME_DELTA ч."
    exit 1
fi

echo "$(date '+%Y-%m-%d %H:%M:%S') - Проверка аудита грабберов завершена успешно. Сообщения были отправлены."
send_telegram_warning_message "✅ $(date '+%Y-%m-%d %H:%M:%S') - Проверка аудита грабберов завершена успешно. 🔔 Сообщения были отправлены в чаты.✈️ "
